...
function StoreAdmin() {
  ...
  return <div className="space-y-5">
    <Card className="border-blue-800 bg-[#141414] text-white"><CardHeader><CardTitle className="text-white">Vendas pelo site</CardTitle></CardHeader><CardContent><div className="mb-5 grid gap-3 sm:grid-cols-4"><div className="rounded-md bg-[#202020] p-3"><p className="text-xs text-neutral-500">Pedidos</p><p className="text-2xl font-black">{sales?.stats.total || 0}</p></div><div className="rounded-md bg-[#202020] p-3"><p className="text-xs text-neutral-500">Pagos</p><p className="text-2xl font-black text-emerald-400">{sales?.stats.approved || 0}</p></div><div className="rounded-md bg-[#202020] p-3"><p className="text-xs text-neutral-500">Pendentes</p><p className="text-2xl font-black text-amber-300">{sales?.stats.pending || 0}</p></div><div className="rounded-md bg-[#202020] p-3"><p className="text-xs text-neutral-500">Faturamento</p><p className="text-2xl font-black text-blue-300">R$ {(sales?.stats.revenue || 0).toFixed(2).replace(".", ",")}</p></div></div><div className="space-y-2">{sales?.clients.filter((order: any) => order.client).map((order: any) => <div key={order.id} className="flex flex-col justify-between gap-3 rounded-md border border-neutral-800 bg-[#202020] p-3 sm:flex-row sm:items-center"><div><p className="font-bold">{order.client.username} <span className="ml-2 text-xs font-normal text-blue-300">{order.productName}</span></p><p className="text-xs text-neutral-500">Pedido #{order.id} · {order.status} · {order.createdAt ? new Date(order.createdAt).toLocaleString("pt-BR") : ""}</p></div><div className="flex flex-wrap gap-2"><Button size="sm" variant="outline" className="border-amber-700 bg-transparent text-amber-300" onClick={() => resetStoreSession.mutate({ userId: order.client.id })}>Resetar sessão</Button><Button size="sm" variant="outline" className="border-blue-700 bg-transparent text-blue-300" onClick={() => toggleStoreClient.mutate({ userId: order.client.id })}>{order.client.isActive ? "Bloquear" : "Ativar"}</Button><Button size="sm" variant="outline" className="border-red-800 bg-transparent text-red-300" onClick={() => deleteStoreClient.mutate({ userId: order.client.id })}>Excluir</Button></div></div>)}</div></CardContent></Card>
    ...
    <div className="space-y-6">
      ...
      <Tabs value={activeSection} onValueChange={setActiveSection} className="space-y-4">
        ...
        <TabsContent value="proxyAndroidClientesKeys" className="space-y-4">
          <Card className="bg-lime-950/30 border-lime-800 text-white"><CardHeader><CardTitle className="text-white">Keys do Proxy Android Clientes</CardTitle></CardHeader><CardContent><p className="text-sm text-lime-100">Disponíveis: <strong>{keysList?.filter(k => k.type === "proxy_android_clientes" && !k.isUsed && !k.isBanned).length || 0}</strong> · Usadas: <strong>{keysList?.filter(k => k.type === "proxy_android_clientes" && (k.isUsed || k.isBanned)).length || 0}</strong></p>{(keysList?.filter(k => k.type === "proxy_android_clientes" && !k.isUsed && !k.isBanned).length || 0) <= 5 && <p className="mt-2 rounded border border-amber-700 bg-amber-950/40 px-3 py-2 text-xs font-bold text-amber-300">Aviso: o estoque de Keys do Proxy Android Clientes está acabando.</p>}<div className="mt-3 max-h-80 overflow-y-auto space-y-1">{keysList?.filter(k => k.type === "proxy_android_clientes").map(k => <div key={k.id} className="flex items-center justify-between border-b border-neutral-800 py-2 text-xs"><span className="font-mono text-cyan-300">{keysRevealed ? k.keyValue : "••••••••••••"}</span><div className="flex items-center gap-2"><Badge className={k.isUsed ? "bg-amber-950 text-amber-400 border-amber-800" : "bg-emerald-950 text-emerald-400 border-emerald-800"}>{k.isUsed ? "Usada" : "Disponível"}</Badge><Button size="sm" variant="destructive" title="Excluir Key do Proxy Android Clientes" onClick={() => { if (confirm(`Excluir a Key ${k.keyValue}? O login existente continuará salvo, mas esta Key será removida da lista.`)) deleteAndroidKeyMutation.mutate({ keyId: k.id }); }}><Trash2 className="w-3 h-3" /></Button></div></div>)}</div></CardContent></Card>
        </TabsContent>
        {/* PAINEL LEGÍTIMO */}
