...
function getVideoEmbedUrl(rawUrl: string): { kind: "iframe" | "video"; url: string } | null {
  try {
    ...
    if (host === "youtube.com" || host === "m.youtube.com" || host === "youtube-nocookie.com") {
      ...
      if (id) return { kind: "iframe", url: `https://www.youtube-nocookie.com/embed/${id}?rel=0` };
    }
    if (host === "vimeo.com") {
...
function ManagementShell({ user, securityHidden, onLogout, children }: { user: ManagementUser; securityHidden: boolean; onLogout: () => void; children: React.ReactNode }) {
  const [activeSection, setActiveSection] = useState(user.role === "moderator" ? "store" : "clients");
...
function StoreAdmin() {
  ...
  const [salesProductFilter, setSalesProductFilter] = useState<"all" | "not_logged" | ActiveProductType>("all");
  const visibleSales = useMemo(() => (sales?.clients || []).filter((order: any) => {
    if (!order.client) return false;
    const matchesSearch = `${order.client.username} ${order.productName || ""} ${order.id}`.toLowerCase().includes(salesSearch.trim().toLowerCase());
    const matchesProduct = salesProductFilter === "all" || (salesProductFilter === "not_logged" ? Boolean(order.client && !order.client.hasLoggedIn) : order.productType === salesProductFilter);
    return matchesSearch && matchesProduct;
  }), [sales, salesProductFilter, salesSearch]);
  ...
  const resetStorePassword = trpc.moderator.resetUserPassword.useMutation({ onSuccess: () => { toast.success("Senha alterada com sucesso e sessões resetadas."); utils.store.salesDashboard.invalidate(); }, onError: (e) => toast.error(e.message) });
  const updateStoreUsername = trpc.moderator.updatePurchasedClientUsername.useMutation({ onSuccess: () => { toast.success("Nome de usuário alterado e sessões resetadas."); utils.store.salesDashboard.invalidate(); }, onError: (e) => toast.error(e.message) });
  const [token, setToken] = useState("");
  const [form, setForm] = useState<{ id?: number; name: string; type: "advanced" | "ios" | "ios_ipa" | "panel_ios" | "panel_android" | "proxy_android_clientes"; category: string; description: string; imageUrl: string; demoType: "none" | "image" | "video"; demoUrl: string; groupLinkUrl: string; price: string; isActive: boolean }>({ name: "", type: "advanced", category: "Proxy", description: "", imageUrl: "", demoType: "none", demoUrl: "", groupLinkUrl: "", price: "", isActive: true });
  const save = trpc.store.saveProduct.useMutation({ onSuccess: () => { toast.success("Produto salvo."); utils.store.listAdminProducts.invalidate(); setForm({ name: "", type: "advanced", category: "Proxy", description: "", imageUrl: "", demoType: "none", demoUrl: "", groupLinkUrl: "", price: "", isActive: true }); }, onError: (e) => toast.error(e.message) });
  ...
  return <div className="space-y-5">
    <Card className="border-blue-800 bg-[#141414] text-white"><CardHeader><CardTitle className="text-white">Vendas pelo site</CardTitle><Input value={salesSearch} onChange={(e) => setSalesSearch(e.target.value)} placeholder="Pesquisar login, produto ou pedido" className="mt-3 border-neutral-700 bg-[#222] text-white" /><div className="-mx-1 mt-3 flex gap-2 overflow-x-auto px-1 pb-1 [&>button]:shrink-0"><Button size="sm" variant={salesProductFilter === "all" ? "default" : "outline"} className={salesProductFilter === "all" ? "bg-red-600 text-white" : "border-neutral-700 bg-transparent text-white"} onClick={() => setSalesProductFilter("all")}>Todos</Button><Button size="sm" variant={salesProductFilter === "not_logged" ? "default" : "outline"} className={salesProductFilter === "not_logged" ? "bg-amber-600 text-white" : "border-neutral-700 bg-transparent text-white"} onClick={() => setSalesProductFilter("not_logged")}>Ainda não entrou</Button>{ACTIVE_PRODUCT_TYPES.map((type) => <Button key={type} size="sm" variant={salesProductFilter === type ? "default" : "outline"} className={salesProductFilter === type ? "bg-blue-600 text-white" : "border-neutral-700 bg-transparent text-white"} onClick={() => setSalesProductFilter(type)}>{PRODUCT_LABELS[type]}</Button>)}</div></CardHeader><CardContent><div className="mb-5 grid grid-cols-2 gap-2 sm:grid-cols-4 sm:gap-3"><div className="rounded-md bg-[#202020] p-3"><p className="text-xs text-neutral-500">Pedidos</p><p className="text-2xl font-black">{sales?.stats.total || 0}</p></div><div className="rounded-md bg-[#202020] p-3"><p className="text-xs text-neutral-500">Pagos</p><p className="text-2xl font-black text-emerald-400">{sales?.stats.approved || 0}</p></div><div className="rounded-md bg-[#202020] p-3"><p className="text-xs text-neutral-500">Pendentes</p><p className="text-2xl font-black text-amber-300">{sales?.stats.pending || 0}</p></div><div className="rounded-md bg-[#202020] p-3"><p className="text-xs text-neutral-500">Faturamento</p><p className="text-2xl font-black text-blue-300">R$ {(sales?.stats.revenue || 0).toFixed(2).replace(".", ",")}</p></div></div><div className="space-y-2">{visibleSales.map((order: any) => <div key={order.id} className="flex flex-col justify-between gap-3 rounded-md border border-neutral-800 bg-[#202020] p-3 sm:flex-row sm:items-center"><div><p className="font-bold">{order.client?.username || order.username} <span className="ml-2 text-xs font-normal text-blue-300">{PRODUCT_LABELS[order.productType] || order.productName}</span></p><p className="text-xs text-neutral-500">Pedido #{order.id} · {order.status} · {order.createdAt ? new Date(order.createdAt).toLocaleString("pt-BR") : ""}</p><p className="mt-1 break-all font-mono text-xs text-cyan-300">Key: {order.key?.keyValue || "Nenhuma Key vinculada"}</p><p className={`mt-1 text-xs font-bold ${order.client?.hasLoggedIn ? "text-emerald-400" : "text-amber-300"}`}>{order.client?.hasLoggedIn ? `Já entrou${order.client.lastLoginAt ? ` · ${new Date(order.client.lastLoginAt).toLocaleString("pt-BR")}` : ""}` : "Ainda não entrou no login"}</p></div><div className="grid gap-2 sm:flex sm:flex-wrap"><Button size="sm" variant="outline" className="min-h-10 w-full border-amber-700 bg-transparent text-amber-300 sm:w-auto" onClick={() => resetStoreSession.mutate({ userId: order.client.id })}>Resetar sessão</Button><Button size="sm" variant="outline" className="border-blue-700 bg-transparent text-blue-300" onClick={() => toggleStoreClient.mutate({ userId: order.client.id })}>{order.client.isActive ? "Bloquear" : "Ativar"}</Button><Button size="sm" variant="outline" className="border-violet-700 bg-transparent text-violet-300" onClick={() => { const newPassword = prompt(`Nova senha para ${order.client.username}:`); if (newPassword && newPassword.length >= 4) resetStorePassword.mutate({ userId: order.client.id, newPassword }); else if (newPassword) toast.error("A senha precisa ter pelo menos 4 caracteres."); }}>Trocar senha</Button><Button size="sm" variant="outline" className="border-cyan-700 bg-transparent text-cyan-300" onClick={() => { const newUsername = prompt(`Novo nome de usuário para ${order.client.username}:`, order.client.username); if (newUsername && newUsername.trim() !== order.client.username) updateStoreUsername.mutate({ userId: order.client.id, newUsername: newUsername.trim() }); }}>Trocar usuário</Button><Button size="sm" variant="outline" className="border-red-800 bg-transparent text-red-300" onClick={() => deleteStoreClient.mutate({ userId: order.client.id })}>Excluir</Button></div></div>)}</div></CardContent></Card>
    ...
    <Card className="border-red-800 bg-[#141414] text-white"><CardHeader><CardTitle className="text-white">Vitrine de produtos</CardTitle></CardHeader><CardContent><p className="mb-4 text-sm text-neutral-400">Cadastre somente produtos que já existem no gerador. A vitrine permanece oculta até ativação.</p><div className="grid gap-3 md:grid-cols-2">{field("name", "Nome do produto")}<select value={form.type} onChange={(e) => setForm((current) => ({ ...current, type: e.target.value as typeof current.type }))} className="h-10 rounded-md border border-neutral-700 bg-[#202020] px-3 text-sm text-white">{[{ value: "advanced", label: "Android Advanced" }, { value: "ios", label: "Proxy iOS" }, { value: "ios_ipa", label: "Proxy iOS IPA" }, { value: "panel_ios", label: "Painel iOS" }, { value: "panel_android", label: "Painel Android" }, { value: "proxy_android_clientes", label: "Proxy Android Clientes" }].map((product) => <option key={product.value} value={product.value}>{product.label}</option>)}</select>{field("category", "Categoria")}{field("price", "Preço em reais (ex.: 19.90)")}{field("imageUrl", "URL HTTPS da imagem")}<select value={form.demoType} onChange={(e) => setForm((current) => ({ ...current, demoType: e.target.value as typeof current.demoType }))} className="h-10 rounded-md border border-neutral-700 bg-[#202020] px-3 text-sm text-white"><option value="none">Sem demonstração</option><option value="image">Demonstração em imagem</option><option value="video">Demonstração em vídeo</option></select>{form.demoType !== "none" && field("demoUrl", form.demoType === "video" ? "URL HTTPS do vídeo" : "URL HTTPS da imagem de demonstração")}{field("groupLinkUrl", "Link HTTPS do grupo (somente compradores deste produto)")}</div><textarea value={form.description} placeholder="Descrição do produto" onChange={(e) => setForm((current) => ({ ...current, description: e.target.value }))} className="mt-3 min-h-24 w-full rounded-md border border-neutral-700 bg-[#202020] p-3 text-sm text-white placeholder:text-neutral-500" /><div className="mt-3 flex gap-3"><Button className="bg-red-600 hover:bg-red-700" disabled={save.isPending} onClick={() => save.mutate({ ...form, price: Number(form.price) })}>{form.id ? "Salvar alterações" : "Adicionar produto"}</Button>{form.id && <Button variant="outline" className="border-neutral-700 bg-transparent text-white" onClick={() => setForm({ name: "", type: "advanced", category: "Proxy", description: "", imageUrl: "", demoType: "none", demoUrl: "", groupLinkUrl: "", price: "", isActive: true })}>Cancelar</Button>}</div><div className="mt-6 space-y-2">{products?.map((product: any) => <div key={product.id} className="flex flex-col justify-between gap-3 rounded-md border border-neutral-800 bg-[#202020] p-3 sm:flex-row sm:items-center"><div className="min-w-0"><p className="font-bold text-white">{product.name} <span className="ml-2 text-xs font-normal text-emerald-400">R$ {Number(product.price).toFixed(2)}</span></p><p className="text-xs text-neutral-500">{product.category} · {product.type} · {product.isActive ? "Ativo" : "Oculto"}</p></div><Button size="sm" variant="outline" className="border-blue-700 bg-transparent text-blue-300" onClick={() => setForm({ id: product.id, name: product.name, type: product.type, category: product.category, description: product.description, imageUrl: product.imageUrl || "", demoType: product.demoType || "none", demoUrl: product.demoUrl || "", groupLinkUrl: product.groupLinkUrl || "", price: String(product.price), isActive: product.isActive })}>Editar</Button><Button size="sm" variant="outline" className="border-red-800 bg-transparent text-red-300 hover:bg-red-950" onClick={() => remove.mutate({ id: product.id })}>Excluir</Button></div>)}</div></CardContent></Card>
  </div>;
}
...
function ModeratorDashboard() {
  ...
  const [modClientKeyId, setModClientKeyId] = useState<number | "">("");
  ...
  const availableManualKeys = (keysList || []).filter((key: any) => key.type === modClientType && key.isActive && !key.isUsed && !key.isBanned);
            ...
            <CardContent>
              <div className="flex gap-4 items-end flex-wrap">
                <div>
                  ...
                  <select className="w-full bg-[#222] border border-neutral-700 rounded p-2 text-white text-sm" value={modClientType} onChange={(e: any) => { setModClientType(e.target.value); setModClientKeyId(""); }}>
                        <option value="advanced">Android Advanced</option>
                        <option value="panel_ios">Painel iOS</option>
                    <option value="panel_android">Painel Android</option>
                    <option value="proxy_android_clientes">Proxy Android Clientes</option>
                ...
                <div className="min-w-[220px]">
                  <label className="text-xs text-white font-semibold block mb-1">Key do login (opcional)</label>
                  <select className="w-full bg-[#222] border border-neutral-700 rounded p-2 text-white text-sm font-mono" value={modClientKeyId} onChange={(e) => setModClientKeyId(e.target.value ? Number(e.target.value) : "")}>
                    <option value="">Automática — primeira disponível</option>
                    {availableManualKeys.map((key: any) => <option key={key.id} value={key.id}>{key.keyValue}</option>)}
                  </select>
                  <p className="mt-1 text-[10px] text-neutral-500">Somente Keys disponíveis de {modClientType}.</p>
                </div>
                <div>
                ...
                <Button className="bg-red-600 hover:bg-red-700 text-white" onClick={() => modCreateClientMutation.mutate({ username: modClientUser, password: modClientPass, type: modClientType, maxDevices: modClientMaxDevices, keyId: modClientKeyId || undefined })}>
          ...
          <Card className="bg-[#141414] border-lime-800 text-white">
            ...
            <CardContent className="space-y-3">
              ...
              {modBulkCredentials.length > 0 && <div className="space-y-2 rounded border border-neutral-700 bg-black/30 p-3">
                <div className="flex flex-wrap items-center justify-between gap-2"><p className="text-sm font-bold text-lime-300">Logins gerados</p><Button size="sm" variant="outline" className="border-lime-700 text-lime-300" onClick={() => { navigator.clipboard.writeText(modBulkCredentials.map((credential) => `Usuário: ${credential.username}\nSenha: ${credential.password}\nLink de ativação: https://shelbyxits.up.railway.app`).join("\n\n")); toast.success("Todos os logins foram copiados!"); }}><Copy className="w-3 h-3 mr-1" /> Copiar todos</Button></div>
                <div className="max-h-72 overflow-y-auto space-y-2">{modBulkCredentials.map((credential) => <div key={`${credential.username}-${credential.password}`} className="rounded border border-neutral-800 bg-[#1b1b1b] p-2 font-mono text-xs"><div>Usuário: <strong className="text-white">{credential.username}</strong></div><div>Senha: <strong className="text-white">{credential.password}</strong></div><div>Link: <span className="text-blue-400">https://shelbyxits.up.railway.app</span></div><Button size="sm" variant="outline" className="mt-2 border-neutral-700 text-neutral-200" onClick={() => { navigator.clipboard.writeText(`Usuário: ${credential.username}\nSenha: ${credential.password}\nLink de ativação: https://shelbyxits.up.railway.app`); toast.success("Login copiado!"); }}><Copy className="w-3 h-3 mr-1" /> Copiar</Button></div>)}</div>
              </div>}
            </CardContent>
          </Card>
          <Card className="bg-[#141414] border-neutral-800 text-white">
...
function ClientDashboard() {
  ...
  return (
    <>
      ...
      {(data?.purchasedGroups || []).length > 0 && <Card className="border-emerald-700/60 bg-[#141414] text-white shadow-xl"><CardHeader><CardTitle className="text-white" style={{ color: brandColor }}>Grupos exclusivos dos seus produtos</CardTitle><p className="text-xs text-neutral-400">Estes links aparecem somente porque seu pagamento foi aprovado para o produto correspondente.</p></CardHeader><CardContent className="space-y-3">{(data?.purchasedGroups || []).map((group: { productId: number; productName: string; url: string }) => <div key={group.productId} className="flex flex-col gap-3 rounded-lg border border-neutral-800 bg-[#1f1f1f] p-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-bold text-white">{group.productName}</p><p className="text-xs text-neutral-400">Grupo exclusivo para compradores</p></div><a href={group.url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center justify-center rounded-md px-4 py-2 text-sm font-bold text-white" style={{ backgroundColor: brandColor }}>Entrar no grupo</a></div>)}</CardContent></Card>}
      {/* TUTORIAIS */}
