...
export const appRouter = router({
  ...
  store: router({
    ...
    cancelStoreOrder: publicProcedure.input(z.object({ paymentId: z.string().min(1) })).mutation(async ({ input }) => { const db = await getDb(); if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR" }); const order = (await db.select().from(storeOrders).where(eq(storeOrders.paymentId, input.paymentId)).limit(1))[0]; if (!order) throw new TRPCError({ code: "NOT_FOUND" }); if (order.status === "approved") throw new TRPCError({ code: "BAD_REQUEST", message: "Este pagamento já foi aprovado e não pode ser cancelado por aqui." }); await db.update(storeOrders).set({ status: "cancelled" }).where(eq(storeOrders.id, order.id)); return { success: true }; }),
    cancelResellerSignup: publicProcedure.input(z.object({ orderId: z.number() })).mutation(async ({ input }) => { const db = await getDb(); if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR" }); const order = (await db.select().from(resellerSignupOrders).where(eq(resellerSignupOrders.id, input.orderId)).limit(1))[0]; if (!order) throw new TRPCError({ code: "NOT_FOUND" }); if (order.status === "approved" || order.createdUserId) throw new TRPCError({ code: "BAD_REQUEST", message: "Este pagamento já foi aprovado e não pode ser cancelado por aqui." }); await db.update(resellerSignupOrders).set({ status: "cancelled" }).where(eq(resellerSignupOrders.id, order.id)); return { success: true }; }),
    cancelCreditOrder: protectedProcedure.input(z.object({ orderId: z.number() })).mutation(async ({ input, ctx }) => { if (ctx.user.role !== "reseller") throw new TRPCError({ code: "FORBIDDEN" }); const db = await getDb(); if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR" }); const order = (await db.select().from(resellerCreditOrders).where(and(eq(resellerCreditOrders.id, input.orderId), eq(resellerCreditOrders.resellerId, ctx.user.id))).limit(1))[0]; if (!order) throw new TRPCError({ code: "NOT_FOUND" }); if (order.status === "approved") throw new TRPCError({ code: "BAD_REQUEST", message: "Este pagamento já foi aprovado e não pode ser cancelado por aqui." }); await db.update(resellerCreditOrders).set({ status: "cancelled" }).where(eq(resellerCreditOrders.id, order.id)); return { success: true }; }),
    ...
    resellerSignupCheckout: publicProcedure.input(z.object({ planId: z.number(), email: z.string().email() })).mutation(async ({ input }) => {
      const db = await getDb(); if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR" });
      const plan = (await db.select().from(resellerPlans).where(and(eq(resellerPlans.id, input.planId), eq(resellerPlans.isActive, true))).limit(1))[0];
      if (!plan) throw new TRPCError({ code: "NOT_FOUND", message: "Plano não encontrado." });
      const settings = await db.select().from(storeSettings).limit(1); const token = settings[0]?.mercadoPagoToken || process.env.MERCADOPAGO_ACCESS_TOKEN;
      if (!token) throw new TRPCError({ code: "PRECONDITION_FAILED", message: "O pagamento ainda não foi configurado pelo administrador." });
      const reference = `reseller_${randomBytes(12).toString("hex")}`;
      await db.insert(resellerSignupOrders).values({ externalReference: reference, planId: plan.id, email: input.email, status: "pending" });
      const response = await fetch("https://api.mercadopago.com/v1/payments", { method: "POST", headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json", "X-Idempotency-Key": reference }, body: JSON.stringify({ transaction_amount: Number(plan.price), description: plan.name, payment_method_id: "pix", external_reference: reference, payer: { email: input.email } }) });
      if (!response.ok) { await db.update(resellerSignupOrders).set({ status: "checkout_failed" }).where(eq(resellerSignupOrders.externalReference, reference)); throw new TRPCError({ code: "BAD_GATEWAY", message: "Mercado Pago recusou o pagamento do plano." }); }
      const payment: any = await response.json(); await db.update(resellerSignupOrders).set({ paymentId: String(payment.id), status: payment.status || "pending" }).where(eq(resellerSignupOrders.externalReference, reference));
      const transactionData = payment.point_of_interaction?.transaction_data; if (!transactionData?.qr_code || !transactionData?.qr_code_base64) throw new TRPCError({ code: "BAD_GATEWAY", message: "O Mercado Pago não retornou o Pix." });
      return { orderId: (await db.select({ id: resellerSignupOrders.id }).from(resellerSignupOrders).where(eq(resellerSignupOrders.externalReference, reference)).limit(1))[0].id, paymentId: String(payment.id), qrCode: transactionData.qr_code, qrCodeBase64: transactionData.qr_code_base64 };
    }),
    resellerSignupStatus: publicProcedure.input(z.object({ orderId: z.number() })).query(async ({ input }) => {
      const db = await getDb(); if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR" }); const order = (await db.select().from(resellerSignupOrders).where(eq(resellerSignupOrders.id, input.orderId)).limit(1))[0]; if (!order) throw new TRPCError({ code: "NOT_FOUND" });
      const settings = await db.select().from(storeSettings).limit(1); const token = settings[0]?.mercadoPagoToken || process.env.MERCADOPAGO_ACCESS_TOKEN; if (!token || !order.paymentId) return { approved: false, status: order.status };
      const response = await fetch(`https://api.mercadopago.com/v1/payments/${encodeURIComponent(order.paymentId)}`, { headers: { Authorization: `Bearer ${token}` } }); const payment: any = await response.json(); if (payment.status === "approved" && order.status !== "approved") await db.update(resellerSignupOrders).set({ status: "approved" }).where(eq(resellerSignupOrders.id, order.id));
      return { approved: payment.status === "approved", status: payment.status || order.status };
    }),
    completeResellerSignup: publicProcedure.input(z.object({ orderId: z.number(), username: z.string().trim().min(3).max(64).regex(/^[a-zA-Z0-9_.-]+$/), password: z.string().min(4).max(128), discordUrl: z.string().trim().max(512) })).mutation(async ({ input }) => {
      const db = await getDb(); if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR" }); const order = (await db.select().from(resellerSignupOrders).where(eq(resellerSignupOrders.id, input.orderId)).limit(1))[0]; if (!order || order.status !== "approved") throw new TRPCError({ code: "PRECONDITION_FAILED", message: "O pagamento ainda não foi aprovado." }); if (order.createdUserId) return { success: true, username: input.username };
      const existing = await db.select({ id: users.id }).from(users).where(eq(users.openId, input.username)).limit(1); if (existing.length) throw new TRPCError({ code: "CONFLICT", message: "Esse nome de usuário já está em uso." }); const plan = (await db.select().from(resellerPlans).where(eq(resellerPlans.id, order.planId)).limit(1))[0]; const discordUrl = validateDiscordUrl(input.discordUrl); const result = await db.insert(users).values({ openId: input.username, email: order.email, role: "reseller", passwordHash: hashPassword(input.password) as any, creditsAdvanced: plan?.initialCredits || 0, credits: plan?.initialCredits || 0, isPremium: plan?.isPremium || false, resellerDiscordUrl: discordUrl, isActive: true, enabledProducts: JSON.stringify([...ACTIVE_PRODUCT_TYPES]) }); const userId = Number(result[0]?.insertId || 0); await db.update(resellerSignupOrders).set({ createdUserId: userId }).where(eq(resellerSignupOrders.id, order.id)); return { success: true, username: input.username };
    }),
    createCreditCheckout: protectedProcedure.input(z.object({ productType: z.enum(STORE_PRODUCT_TYPES), quantity: z.number().int().min(1).max(1000) })).mutation(async ({ input, ctx }) => {
      if (ctx.user.role !== "reseller") throw new TRPCError({ code: "FORBIDDEN", message: "Somente revendedores podem comprar créditos." }); const db = await getDb(); if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR" }); const product = (await db.select().from(storeProducts).where(and(eq(storeProducts.type, input.productType), eq(storeProducts.isActive, true))).limit(1))[0]; const productNames: Record<string, string> = { advanced: "Proxy", ios: "Proxy iOS", ios_ipa: "Proxy iOS IPA", panel_ios: "Painel iOS", panel_android: "Painel Android", proxy_android_clientes: "Proxy Android Clientes" }; const visibilitySettings = (await db.select().from(storeSettings).limit(1))[0]; let visibleProducts: Record<string, boolean> = {}; let creditProducts: Record<string, boolean> = {}; try { visibleProducts = visibilitySettings?.resellerVisibleProducts ? JSON.parse(visibilitySettings.resellerVisibleProducts) : {}; } catch {} try { creditProducts = visibilitySettings?.resellerCreditProducts ? JSON.parse(visibilitySettings.resellerCreditProducts) : {}; } catch {} if (creditProducts[input.productType] === false) throw new TRPCError({ code: "FORBIDDEN", message: "Este produto não está liberado para compra de créditos." }); const settings = await db.select().from(storeSettings).limit(1); let creditValues: Record<string, string> = {}; try { creditValues = settings[0]?.resellerCreditValues ? JSON.parse(settings[0].resellerCreditValues) : {}; } catch {} const configuredUnitPrice = Number(String(creditValues[input.productType] || product.price).replace(",", ".")); if (!Number.isFinite(configuredUnitPrice) || configuredUnitPrice <= 0) throw new TRPCError({ code: "PRECONDITION_FAILED", message: "Configure um valor válido por Key para este produto no painel moderador." }); const token = settings[0]?.mercadoPagoToken || process.env.MERCADOPAGO_ACCESS_TOKEN; if (!token) throw new TRPCError({ code: "PRECONDITION_FAILED", message: "Mercado Pago não configurado." }); const reference = `credits_${randomBytes(12).toString("hex")}`; await db.insert(resellerCreditOrders).values({ externalReference: reference, resellerId: ctx.user.id, productType: input.productType, quantity: input.quantity, unitPrice: configuredUnitPrice.toFixed(2), status: "pending" }); const response = await fetch("https://api.mercadopago.com/v1/payments", { method: "POST", headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json", "X-Idempotency-Key": reference }, body: JSON.stringify({ transaction_amount: configuredUnitPrice * input.quantity, description: `${input.quantity} créditos — ${product?.name || productNames[input.productType] || input.productType}`, payment_method_id: "pix", external_reference: reference, payer: { email: ctx.user.email || `${ctx.user.openId}@example.com` } }) }); if (!response.ok) throw new TRPCError({ code: "BAD_GATEWAY", message: "Mercado Pago recusou a compra de créditos." }); const payment: any = await response.json(); await db.update(resellerCreditOrders).set({ paymentId: String(payment.id), status: payment.status || "pending" }).where(eq(resellerCreditOrders.externalReference, reference)); const transactionData = payment.point_of_interaction?.transaction_data; if (!transactionData?.qr_code || !transactionData?.qr_code_base64) throw new TRPCError({ code: "BAD_GATEWAY", message: "O Mercado Pago não retornou o Pix." }); return { orderId: (await db.select({ id: resellerCreditOrders.id }).from(resellerCreditOrders).where(eq(resellerCreditOrders.externalReference, reference)).limit(1))[0].id, paymentId: String(payment.id), qrCode: transactionData.qr_code, qrCodeBase64: transactionData.qr_code_base64 };
    }),
    creditPaymentStatus: protectedProcedure.input(z.object({ orderId: z.number() })).query(async ({ input, ctx }) => {
      if (ctx.user.role !== "reseller") throw new TRPCError({ code: "FORBIDDEN" }); const db = await getDb(); if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR" }); const order = (await db.select().from(resellerCreditOrders).where(and(eq(resellerCreditOrders.id, input.orderId), eq(resellerCreditOrders.resellerId, ctx.user.id))).limit(1))[0]; if (!order) throw new TRPCError({ code: "NOT_FOUND" }); const settings = await db.select().from(storeSettings).limit(1); const token = settings[0]?.mercadoPagoToken || process.env.MERCADOPAGO_ACCESS_TOKEN; if (!token || !order.paymentId) return { approved: false, status: order.status }; const response = await fetch(`https://api.mercadopago.com/v1/payments/${encodeURIComponent(order.paymentId)}`, { headers: { Authorization: `Bearer ${token}` } }); const payment: any = await response.json(); if (payment.status === "approved" && order.status !== "approved") { const columns: Record<string, string> = { advanced: "creditsAdvanced", ios: "creditsIos", ios_ipa: "creditsIosIpa", panel_ios: "creditsPanelIos", panel_android: "creditsPanelAndroid", proxy_android_clientes: "creditsProxyAndroidClientes" }; const column = columns[order.productType]; const reseller = (await db.select().from(users).where(eq(users.id, ctx.user.id)).limit(1))[0]; if (column && reseller) await db.update(users).set({ [column]: Number((reseller as any)[column] || 0) + order.quantity, credits: Number(reseller.credits || 0) + order.quantity }).where(eq(users.id, ctx.user.id)); await db.update(resellerCreditOrders).set({ status: "approved" }).where(eq(resellerCreditOrders.id, order.id)); } return { approved: payment.status === "approved", status: payment.status || order.status, quantity: order.quantity, productType: order.productType };
    }),
  ...
  moderator: router({
    getResellerConfig: protectedProcedure.query(async ({ ctx }) => {
      if (ctx.user.role !== "moderator") throw new TRPCError({ code: "FORBIDDEN" }); const db = await getDb(); if (!db) return { creditValues: {}, visibleProducts: {}, creditProducts: {} }; const row = (await db.select().from(storeSettings).limit(1))[0]; let creditValues: Record<string, string> = {}; let visibleProducts: Record<string, boolean> = {}; let creditProducts: Record<string, boolean> = {}; try { creditValues = row?.resellerCreditValues ? JSON.parse(row.resellerCreditValues) : {}; } catch {} try { visibleProducts = row?.resellerVisibleProducts ? JSON.parse(row.resellerVisibleProducts) : {}; } catch {} try { creditProducts = row?.resellerCreditProducts ? JSON.parse(row.resellerCreditProducts) : {}; } catch {} return { creditValues, visibleProducts, creditProducts };
    }),
    saveResellerConfig: protectedProcedure.input(z.object({ creditValues: z.record(z.string(), z.string()), visibleProducts: z.record(z.string(), z.boolean()), creditProducts: z.record(z.string(), z.boolean()) })).mutation(async ({ input, ctx }) => {
      if (ctx.user.role !== "moderator") throw new TRPCError({ code: "FORBIDDEN" }); const db = await getDb(); if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR" }); const current = await db.select({ id: storeSettings.id }).from(storeSettings).limit(1); const values = { resellerCreditValues: JSON.stringify(input.creditValues), resellerVisibleProducts: JSON.stringify(input.visibleProducts), resellerCreditProducts: JSON.stringify(input.creditProducts) }; if (current.length) await db.update(storeSettings).set(values).where(eq(storeSettings.id, current[0].id)); else await db.insert(storeSettings).values(values); return { success: true };
    }),
  ...
  reseller: router({
    ...
    createClient: protectedProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        const actorRes = await db.select().from(users).where(eq(users.id, ctx.user.id)).limit(1);
        const actor = actorRes[0];
        const globalSettings = (await db.select().from(storeSettings).limit(1))[0];
        let globalVisible: Record<string, boolean> = {};
        try { globalVisible = globalSettings?.resellerVisibleProducts ? JSON.parse(globalSettings.resellerVisibleProducts) : {}; } catch {}
        if (ctx.user.role === "reseller" && globalVisible[input.type] === false) throw new TRPCError({ code: "FORBIDDEN", message: "Este produto está oculto pelo moderador e não pode gerar logins." });
