  ...
  return (
    <div className="space-y-6">
      ...
      {(() => {
        ...
        return lowStock.length > 0 ? <Card className="bg-red-950/40 border-red-700 text-white"><CardContent className="p-4"><div className="flex items-start gap-3"><AlertTriangle className="w-6 h-6 text-red-400 mt-0.5 shrink-0" /><div className="w-full"><p className="font-black text-lg text-red-300">ATENÇÃO: estoque de Keys acabando</p><p className="text-sm font-bold text-red-100 mt-1">Total nos painéis em alerta: <strong className="text-xl">{lowStockTotal} Keys</strong></p><p className="text-xs text-red-200/80 mt-1">Cada painel abaixo possui 3 Keys ou menos disponíveis. Reponha o estoque.</p><div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 mt-3">{lowStock.map(([type, label]) => <div key={type} className="rounded-md border border-red-600 bg-red-900/60 px-3 py-2"><p className="text-xs font-bold text-red-100">{label}</p><p className="text-lg font-black text-white">{Number((stats?.stock as any)?.[type] || 0)} Keys disponíveis</p></div>)}</div></div></div></CardContent></Card> : null;
      })()}
